...
export function analyzeWorkflowSteps(steps: WorkflowSteps): WorkflowAnalysis {
  ...
  for (const stepName of stepNames) {
    ...
    stepArray.forEach((step, index) => {
      ...
      // Add to timeline
      const executionLabel = isRepeated ? `${stepName}[${index + 1}]` : stepName;
      if (step.startedAt) {
        timeline.push(`[${new Date(step.startedAt).toISOString()}] ${executionLabel} started`);
      }
      if (step.endedAt) {
        const dur = formatDuration(duration);
        timeline.push(`[${new Date(step.endedAt).toISOString()}] ${executionLabel} completed (${dur})`);
      }
    });
  ...
  const totalDuration = earliestStart !== Infinity && latestEnd !== 0 
...
export function printWorkflowAnalysis(analysis: WorkflowAnalysis): void {
  ...
  if (analysis.totalExecutions > analysis.totalSteps) {
    console.log(`  Total Executions: ${analysis.totalExecutions} (some steps repeated)`);
  }
  ...
  analysis.stepDetails.forEach(step => {
    ...
    const countSuffix = step.executionCount && step.executionCount > 1 
      ? ` (${step.executionCount}x)` 
      : '';
    console.log(`  ${statusIcon} ${step.name}${countSuffix}: ${step.durationFormatted}`);
  ...
  if (analysis.keyOutputs.refinedSummary && analysis.keyOutputs.refinedSummary !== analysis.keyOutputs.summary) {
    console.log(`  Refined Summary: ${analysis.keyOutputs.refinedSummary.slice(0, 150)}...`);
  }
