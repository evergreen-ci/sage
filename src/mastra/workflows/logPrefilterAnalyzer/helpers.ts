...
export const generateMarkdownAndSummary = async ({
  ...
  logger.debug('Generating markdown report from pre-filtered analysis', {
    textLength: text.length,
  });

  const markdownResult = await agent.generate(
    USER_MARKDOWN_PROMPT(text, scanResult, analysisContext),
    {
      abortSignal,
      ...context,
    }
  ...
  try {
    ...
    logger.debug('Failed to parse line references, using existing', {
      error,
    });
  }
