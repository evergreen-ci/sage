...
describe('getImageWorkflow', () => {
  ...
  describe('with distroId', () => {
    it('success: returns full image data from distroId', async () => {
      ...
      // Check if it's full image (not changes-only)
    ...
    it('success: returns only changes from distroId when changesOnly is true', async () => {
      const mockDistroData = {
        distro: {
          name: 'ubuntu2204-small',
          imageId: 'image-123',
      ...
      expect(image.id).toBe('image-123');
      expect(image.ami).toBe('ami-abc123');
      // Check if it's changes-only format
      if ('recentChanges' in image) {
        expect(image.recentChanges).toBeDefined();
        expect(image.recentChanges).toHaveLength(1);
        expect('packages' in image).toBe(false);
        expect('toolchains' in image).toBe(false);
      }
    });
  });
  ...
  describe('with taskId', () => {
    ...
    it('success: returns only changes from taskId when changesOnly is true', async () => {
      const mockTaskData = {
        task: {
          id: 'task-123',
          displayName: 'test-task',
